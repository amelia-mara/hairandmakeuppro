...
export function ContinuityTracker({ projectId }: Props) {
  ...
  /* Mobile-only — phone viewport (≤768px) collapses the two-panel
     layout: scene list slides in from the left as a drawer toggled
     by ☰ in the mobile scene strip. Picking a scene auto-closes
     the drawer. */
  ...
  /* Mobile prev/next walk the unfiltered scenes list so the strip
     arrows always land on a real adjacent scene. */
  ...
  return (
    <div className={`bd-page${isMobile ? ' bd-page--mobile' : ''}`}>
      {/* Mobile-only scene navigator strip */}
      {isMobile && scene && (
