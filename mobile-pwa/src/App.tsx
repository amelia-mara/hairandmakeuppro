...
function AppContent() {
  ...
  // F-04's localStorage prune effect below depends on
  // projectMemberships (kept as a separate subscription so the prune
  // re-runs when memberships rehydrate). Note this snapshot may lag
  // the live server view on initial mount; F-34's boot validation
  // (further down) is the one that *awaits* a fresh fetch for the
  // critical "is the current project still real" decision.
