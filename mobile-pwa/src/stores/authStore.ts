      ...
      joinProject: async (code) => {
        ...
        try {
          if (!isAuthenticated || !user) {
            // Guest mode - look up project, fetch data, and load into project store
            const { project: dbProject, error: lookupError } =
              await supabaseProjects.getProjectByInviteCode(code);

            if (lookupError || !dbProject) {
              set({ isLoading: false, error: lookupError?.message || 'Invalid project code' });
              return { success: false, error: lookupError?.message || 'Invalid project code' };
            }

            // Fetch full project data from Supabase
            const { scenes, characters, looks, sceneCharacters, lookScenes } =
              await supabaseProjects.getProjectData(dbProject.id);

            const projectStore = useProjectStore.getState();

            if (scenes.length > 0 || characters.length > 0) {
              // Build scene-character mapping
              ...
              // No data yet - create empty project shell
              const project: Project = {
                id: dbProject.id,
                name: dbProject.name,
                createdAt: new Date(dbProject.created_at),
            ...
            projectStore.setActiveTab('today');
