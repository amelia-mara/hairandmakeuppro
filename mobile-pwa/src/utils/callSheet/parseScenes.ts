...
function isLikelyNewRow(cells: string[]): boolean {
  ...
  if (cells.slice(sceneIdx + 1).some((c) => /[.,]/.test(c) && CAST_LIST_RE.test(c))) return true;
  return false;
}
...
function consumeRow(cells: string[]): Partial<CallSheetScene> {
  ...
  for (; i < cells.length; i++) {
    ...
    if (!row.pages && PAGES_INT_RE.test(c)) {
      // Look ahead one cell for a fractional companion ("1" + "5/8").
      const nxt = cells[i + 1];
      if (nxt && PAGES_FRAC_RE.test(nxt)) {
        row.pages = `${c} ${nxt}`;
        i += 1;
      } else {
        row.pages = c;
