...
/**
 * Show API limit help for Claude
 */
function showAPILimitHelp() {
    const helpText = `Check your Claude API usage and rate limits:
...
TIP: Claude has generous rate limits on paid tiers. The app will automatically retry with exponential backoff if you hit limits.`;
...
export function openSettingsModal() {
    ...
    const anthropicKey = localStorage.getItem('anthropicApiKey') || state.apiKey || '';
...
export function saveSettings() {
    ...
    showToast('Claude API settings saved successfully', 'success');
    closeSettingsModal();
}

/**
 * Toggle provider-specific settings visibility
 * Note: Now Claude-only, this function is kept for backward compatibility
 */
export function toggleProviderSettings() {
    // No-op: Only Claude is supported now
}

/**
 * Test API connection (Claude only)
 */
export async function testAPIConnection() {
    ...
    const oldModel = state.anthropicModel;
    ...
    try {
        showToast('Testing Claude connection...', 'info');
...
/**
 * Internal AI caller with detailed logging (Claude only)
 */
async function callAIInternal(prompt, maxTokens, sceneNumber) {
    const sceneLabel = sceneNumber !== null ? `Scene ${sceneNumber}` : 'Request';
    console.log(`🤖 Claude AI Call Started - ${sceneLabel}`);
    ...
    if (window.location.hostname.includes('vercel.app')) {
        try {
            ...
            if (data.content && data.content[0]) {
                console.log(`✅ Claude AI Call Success - ${sceneLabel}`);
    ...
    if (!state.apiKey) {
        const error = new Error('No Claude API key configured. Please set up AI Settings in the Tools panel.');
        console.error('❌ No Claude API key configured');
        showErrorNotification('No Claude API key configured. Open AI Settings to add your key.');
        throw error;
    }
    ...
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.error(`❌ Claude API Error (${sceneLabel}):`, errorText);
        ...
        console.error(`💥 Exception in Claude call (${sceneLabel}):`, error);
        throw error;
    }
}
